...
export const useInterview = () => {
  ...
  const cleanupAudio = useCallback(() => {
    if (audioResources.current) {
      try {
        if (audioResources.current.workletNode) {
          audioResources.current.workletNode.port.onmessage = null;
          audioResources.current.workletNode.disconnect();
        }
      } catch (error) {
        console.warn('Error disconnecting workletNode:', error);
      }
      try {
        audioResources.current.gainNode?.disconnect();
      } catch (error) {
  ...
  const endInterview = useCallback(async () => {
    ...
    try {
      ...
      const is429 = err?.code === 429 || err?.status === 429 || (typeof err?.message === 'string' && err.message.includes('quota'));
      if (is429) setQuotaExceededError(true);
      setFeedback({
      ...
      sessionPromise.current = ai.live.connect({
        ...
        callbacks: {
          onopen: async () => {
            ...
            audioResources.current = { inputAudioContext, outputAudioContext, mediaStreamSource, workletNode, gainNode };
          },
          onmessage: async (message: LiveServerMessage) => {
            // Accumulate input transcription from Gemini
            ...
            // On turn complete, add messages to history
            if (message.serverContent?.turnComplete) {
              if (currentInputTranscription) {
                addMessage({ sender: 'USER', text: currentInputTranscription });
              }
              if (currentOutputTranscription) {
                addMessage({ sender: 'AI', text: currentOutputTranscription });
              }
