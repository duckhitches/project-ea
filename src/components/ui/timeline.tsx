...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div className={`w-full bg-transparent md:px-10 font-cabin ${montserrat.className}`} ref={containerRef}>
        ...
        {data.map((item, index) => (
          <div key={index} className="flex justify-start pt-10 md:pt-40 md:gap-10">
            <div className="sticky flex flex-col md:flex-row z-10 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              <div className="h-12 w-12 absolute left-3 md:left-3 rounded-full bg-transparent dark:bg-white/10 flex items-center justify-center shadow-lg">
                <div className="h-10 w-10 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                  {item.icon}
                </div>
              </div>
              <h3 className="hidden md:block text-xl md:pl-20 md:text-3xl font-semibold text-gray-900 dark:text-white">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-3xl sm:text-3xl mb-4 text-left font-semibold text-gray-900 dark:text-white">
                {item.title}
              </h3>
              <div className="bg-white/5 dark:bg-white/5 backdrop-blur-md border border-gray-200 dark:border-white/10 text-sm sm:text-sm md:text-base lg:text-lg xl:text-lg text-gray-900 dark:text-white rounded-2xl p-6 shadow-lg">
                {item.content}
              </div>
            </div>
          </div>
